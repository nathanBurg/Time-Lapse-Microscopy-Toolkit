...
def predict_img(net,
    ...
    img = torch.from_numpy(BasicDataset.preprocess(full_img, scale_factor)) # Creates torch tensor from np array

    img = img.unsqueeze(0)
    img = img.to(device=device, dtype=torch.float32)    # Moves the image to the GPU (or CPU is selected)


    with torch.no_grad():
            output = net(img)

            if classes > 1: # Sets the propper activation function based on the number of classes
